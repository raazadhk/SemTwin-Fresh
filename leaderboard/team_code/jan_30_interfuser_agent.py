...
def draw_instance_boxes(rgb_image, instances_info):
    ...
    for instance in instances_info['instances']:
        ...
        cv2.rectangle(image_with_boxes,
                     ...
                     (bbox['x_min'] + label_size[0], bbox['y_min']), color, -1)
        
        cv2.putText(image_with_boxes, label, (bbox['x_min'], bbox['y_min'] - 5),
...
def draw_tracked_objects(rgb_image, tracking_results, show_trails=True):
    ...
    for track_id, track_info in tracking_results.items():
        ...
        cv2.rectangle(image_with_tracks, (bbox['x_min'], bbox['y_min']),
                     (bbox['x_max'], bbox['y_max']), color, 3)
        ...
        if velocity['speed'] > 1:
            ...
            cv2.arrowedLine(image_with_tracks, tuple(center), arrow_end,
...
class InstanceTracker:
    ...
    def update(self, detections):
        ...
        previous_centers = np.array([self.objects[oid]['history'][-1]['center'] for oid in object_ids])
...
class InterfuserAgent(autonomous_agent.AutonomousAgent):
    def setup(self, path_to_conf_file):
        ...
        # Digital Twin Config (UPDATED FOR SAME MACHINE)
        twin_host = os.environ.get("DIGITAL_TWIN_HOST", None)  # Changed default
        twin_port = int(os.environ.get("DIGITAL_TWIN_PORT", "9999"))
        twin_enabled = os.environ.get("DIGITAL_TWIN_ENABLED", "false").lower() == "true"  # Changed default
        ...
        if twin_enabled and self.twin_broadcaster.connected:
            print(f"✓ Digital Twin connection established (same machine mode)")
    ...
    def run_step(self, input_data, timestamp):
        ...
        # BROADCAST TO DIGITAL TWIN (SAME MACHINE)
        if self.twin_broadcaster.enabled and self.twin_broadcaster.connected:
            try:
                tracking_results = tick_data.get('tracking_results', {}) or {}
                instance_counts = tick_data.get('instance_analysis', {}).get('counts', {})
                
                # Prepare LiDAR and segmentation for environment reconstruction
                lidar_points = tick_data.get('raw_lidar', None)
                if lidar_points is not None:
                    lidar_sampled = lidar_points[::5, :3]  # Subsample every 5th point
                else:
                    lidar_sampled = None
        
                seg_front = tick_data.get('seg_front', None)
                ...
                state_data = {
                    ...
                    'images': {
                     ...
                     # NEW: Environment reconstruction data
                    'lidar_points': lidar_sampled,
