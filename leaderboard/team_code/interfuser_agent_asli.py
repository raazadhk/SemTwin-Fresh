...
class InterfuserAgent(autonomous_agent.AutonomousAgent):
    ...
    def reduce_image_quality(self, image, scale_percent=10):
        ...
        dim = (width, height)
        # Resize down to simulate quality reduction
        ...
        noise_factor = 10  # Adjust this factor to control the noise level
        noise = np.random.randint(0, noise_factor, resized_original.shape, dtype='uint8')
        noisy_image = cv2.add(resized_original, noise)
        
        _, compressed = cv2.imencode('.jpg', noisy_image, [cv2.IMWRITE_JPEG_QUALITY, 5])  # Adjust quality for more compression
    ...
    def apply_random_blockades(self, image, blockade_size=(200, 200), number_of_blockades=20):
        """
        Applies random blockades to the given image.

        Args:
        - image (numpy.ndarray): The input image.
        - blockade_size (tuple): The size of the blockades.
        - number_of_blockades (int): The number of blockades to apply.

        Returns:
        - numpy.ndarray: The modified image with blockades.
        """
    ...
    def tick(self, input_data):
        ...
        rgb_now = cv2.cvtColor(input_data["rgb"][1][:, :, :3], cv2.COLOR_BGR2RGB)
        # Apply random blockades to the rgb camera image only
        rgb = self.apply_random_blockades(rgb_now)
        #rgb= self.reduce_image_quality(rgb_now, scale_percent=5) 
        #cv2.imwrite('reduced_quality_rgb.jpg', cv2.cvtColor(rgb, cv2.COLOR_RGB2BGR))
        #dummy_rgb_dim = (600, 800, 3) 
        #rgb_dummy = np.zeros(dummy_rgb_dim, dtype=np.uint8)
        #rgb = cv2.cvtColor(rgb_dummy, cv2.COLOR_BGR2RGB)
        #Assuming the dimension of the left camera is 400x300, and it's RGB
        #rgb_left_dummy = np.zeros((300, 400, 3), dtype=np.uint8)  # Dummy black image
        rgb_left_now = cv2.cvtColor(input_data["rgb_left"][1][:, :, :3], cv2.COLOR_BGR2RGB)
        rgb_left = self.reduce_image_quality(rgb_left_now, scale_percent=10) 
        #saving image for inspection
        #cv2.imwrite('reduced_quality_rgb_left.jpg', cv2.cvtColor(rgb_left, cv2.COLOR_RGB2BGR))
        #rgb_right = cv2.cvtColor(
         #   input_data["rgb_right"][1][:, :, :3], cv2.COLOR_BGR2RGB
        #)
        #rgb_right_now = cv2.cvtColor(
        #    input_data["rgb_right"][1][:, :, :3], cv2.COLOR_BGR2RGB
        #)
        #rgb_right= self.reduce_image_quality(rgb_now, scale_percent=10)
        # Apply random blockades to the rgb_right camera image only
        #rgb_right = self.apply_random_blockades(rgb_right_now)
        ...
        # Process dummy data for rgb_left similarly to actual data
        #rgb_left = cv2.cvtColor(rgb_left_dummy, cv2.COLOR_BGR2RGB)    
    ...
    def send_frame_to_multiple_jetsons(self, frame, jetson_devices):
        for camera, (ip, port) in jetson_devices.items():
            ...
            #Measure latency
            #latency = self.measure_latency(ip, port)
            ...
            # log bandwidth and throughput 
            #end_time = time.time()
            #self.log_bandwidth(data_size, start_time, end_time)
            ...
            #    if ack == b'ACK':
            #        print(f"ACK received from {ip}:{port}, frame sent successfully.")
            #    else:
            #        print(f"Unexpected response from {ip}:{port}, possible packet loss.")
            #except socket.timeout:
            #    print(f"No ACK received from {ip}:{port}, possible packet loss.")
            #
            #
            client_socket.close()
    
    #def measure_latency(self, ip, port):
        #client_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        #client_socket.connect((ip, port))
    
        #start_time = time.time()
        #client_socket.sendall(b"ping")  # Send a small ping message
    
        #response = client_socket.recv(1024)
        #end_time = time.time()
    
        #latency = end_time - start_time
        #print(f"Latency to {ip}:{port} is {latency * 1000:.2f} ms")
    
        #client_socket.close()
        #return latency  
        
    #def log_bandwidth(self, data_size, start_time, end_time):
    #    duration = end_time - start_time
    #    bandwidth = (data_size / duration) / (1024 * 1024)  # in MBps
    #    print(f"Sent {data_size} bytes in {duration:.2f} seconds, Bandwidth: {bandwidth:.2f} MBps") 
        
    #def calculate_throughput(total_data, start_time, end_time):
        #duration = end_time - start_time
        #throughput = total_data / duration  # in bytes per second
    ...
    def run_step(self, input_data, timestamp):
        ...
        #jetson_devices = {
            #'rgb': ('128.235.248.148', 12345),
            #'rgb_left': ('128.235.248.30', 12346)
            #'rgb_right': ('128.235.248.5', 12347)  # Replace 'XX' with the actual last octet of the third Jetson's IP
        #}
        ...
        # Create dummy data for rgb_center
        # Assuming you know the expected dimensions and color channels (e.g., 3 for RGB)
        #dummy_rgb_center_shape = (1, 3, tick_data["rgb"].shape[0], tick_data["rgb"].shape[1])  # Example shape
        #dummy_rgb_center = torch.zeros(dummy_rgb_center_shape).cuda().float()
        
        # Create dummy data for 'rgb_center' as a black image
        # Determine the expected dimensions for the input, e.g., 224x224 for each channel (RGB)
        # Assuming your model expects RGB images of shape [3, 224, 224]
        #dummy_rgb_center_shape = (1, 3, 224, 224)  # Example shape, adjust based on your model's expectations
        #dummy_rgb_center = torch.zeros(dummy_rgb_center_shape).cuda().float()
        
        #dummy_rgb_center = torch.zeros((1, 3, 224, 224), dtype=torch.float32).cuda()
        # If a specific transform is applied to rgb_center, apply it here to the dummy data
        #dummy_rgb_center_transformed = self.rgb_center_transform(transforms.ToPILImage()(dummy_rgb_center.squeeze(0).cpu()))
        #dummy_rgb_center_transformed = dummy_rgb_center_transformed.unsqueeze(0).cuda()
