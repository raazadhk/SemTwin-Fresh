...
# ============================================================================
try:
    from team_code.carla_hd_map import CARLAHDMapExtractor, HDMapVisualizer
    HD_MAP_AVAILABLE = True
    print("✓ HD Map module imported successfully")
except ImportError as e:
    print(f"⚠ HD Map module not available: {e}")
...
FRAME_DIRS = {
    ...
    "rgb_with_hdmap": os.path.join(DATASET_DIR, "RGB_With_HDMap"),
    "bev": os.path.join(DATASET_DIR, "BEV"),  # NEW: BEV camera frames
    ...
    "hd_map_data": os.path.join(DATASET_DIR, "HD_Map_Data"),
...
def draw_tracked_objects(rgb_image, tracking_results, show_trails=True):
    ...
    for track_id, track_info in tracking_results.items():
        ...
        info_size, _ = cv2.getTextSize(info, cv2.FONT_HERSHEY_SIMPLEX, 0.4, 1)
        
        cv2.rectangle(
            image_with_tracks,
            (bbox['x_min'], bbox['y_min'] - label_size[1] - info_size[1] - 10),
            (bbox['x_min'] + max(label_size[0], info_size[0]) + 5, bbox['y_min']),
            color, -1
        )
...
# ============================================================================
# HD MAP HELPER FUNCTIONS (NEW!)
# ============================================================================
def serialize_hd_map_for_json(hd_map_data):
    """Serialize HD map data for JSON storage and Digital Twin"""
    if not hd_map_data:
        return None
    
    serialized = {}
    
    if 'lane_boundaries' in hd_map_data:
        lb = hd_map_data['lane_boundaries']
        serialized['lane_boundaries'] = {
            'left': [[float(p[0]), float(p[1]), float(p[2])] for p in lb.get('left', [])] if lb.get('left') else [],
            'right': [[float(p[0]), float(p[1]), float(p[2])] for p in lb.get('right', [])] if lb.get('right') else [],
            'center': [[float(p[0]), float(p[1]), float(p[2])] for p in lb.get('center', [])] if lb.get('center') else [],
            'left_marking_type': lb.get('left_marking_type'),
            'right_marking_type': lb.get('right_marking_type')
        }
    
    if 'traffic_lights' in hd_map_data:
        serialized['traffic_lights'] = [
            {
                'id': str(light.get('id', '')),
                ...
                'affected_lanes': light.get('affected_lanes', [])
            }
            for light in hd_map_data['traffic_lights']
        ]
    
    if 'traffic_signs' in hd_map_data:
        serialized['traffic_signs'] = hd_map_data['traffic_signs']
    
    if 'current_lane' in hd_map_data:
        serialized['current_lane'] = hd_map_data['current_lane']
    
    if 'valid_actions' in hd_map_data:
        serialized['valid_actions'] = hd_map_data['valid_actions']
...
class InstanceTracker:
    """Track objects across frames using instance IDs and spatial information"""
    ...
    def update(self, detections):
        ...
        previous_centers = np.array([
            self.objects[oid]['history'][-1]['center'] 
        ...
        distances = np.linalg.norm(
            previous_centers[:, np.newaxis] - current_centers[np.newaxis, :],
            axis=2
        )
...
def save_sensor_data_to_disk(step, tick_data, input_data, frame_dirs):
    ...
    measurements = {
        ...
        'has_hd_map': tick_data.get('has_hd_map', False),
        'has_bev_camera': True,  # NEW
    }
...
class InterfuserAgent(autonomous_agent.AutonomousAgent):
    def setup(self, path_to_conf_file):
        ...
        print("✓ Instance tracker initialized (using pseudo-instances from semantic segmentation)")
        
        # HD MAP INITIALIZATION
        self.hd_map_enabled = HD_MAP_AVAILABLE
        if self.hd_map_enabled:
            self.hd_map_extractor = None
            self.hd_map_visualizer = None
            print("✓ HD Map system will be initialized with CARLA world")
        
        # Digital Twin Config
        twin_host = os.environ.get("DIGITAL_TWIN_HOST", None)
        twin_port = int(os.environ.get("DIGITAL_TWIN_PORT", "9999"))
        twin_enabled = os.environ.get("DIGITAL_TWIN_ENABLED", "false").lower() == "true"
    ...
    def _init(self):
        ...
        # INITIALIZE HD MAP WITH CARLA WORLD
        if self.hd_map_enabled and hasattr(self, '_vehicle'):
            try:
                carla_world = self._vehicle.get_world()
                self.hd_map_extractor = CARLAHDMapExtractor(carla_world)
                ...
                self.hd_map_visualizer = HDMapVisualizer(camera_params)
                print("✓ HD Map Extractor initialized")
                print("✓ HD Map Visualizer initialized")
            except Exception as e:
                print(f"✗ Failed to initialize HD Map: {e}")
                self.hd_map_enabled = False
    ...
    def sensors(self):
        return [
            ...
            {"type": "sensor.camera.semantic_segmentation", "x": 1.3, "y": 0.0, "z": 2.3,
             "roll": 0.0, "pitch": 0.0, "yaw": 0.0, "width": 800, "height": 600, "fov": 100, "id": "seg_front"},
            {"type": "sensor.camera.semantic_segmentation", "x": 1.3, "y": 0.0, "z": 2.3,
             "roll": 0.0, "pitch": 0.0, "yaw": -60.0, "width": 400, "height": 300, "fov": 100, "id": "seg_left"},
            {"type": "sensor.camera.semantic_segmentation", "x": 1.3, "y": 0.0, "z": 2.3,
             "roll": 0.0, "pitch": 0.0, "yaw": 60.0, "width": 400, "height": 300, "fov": 100, "id": "seg_right"},
            
            # ============================================================
            # BEV CAMERA - Bird's Eye View (NEW!)
            # ============================================================
            {"type": "sensor.camera.rgb", "x": 0.0, "y": 0.0, "z": 50.0,
             "roll": 0.0, "pitch": -90.0, "yaw": 0.0, "width": 800, "height": 800, "fov": 90, "id": "bev"},
    ...
    def tick(self, input_data):
        ...
        # ============================================================
        # GET BEV CAMERA IMAGE (NEW!)
        # ============================================================
        bev_raw = cv2.cvtColor(input_data["bev"][1][:, :, :3], cv2.COLOR_BGR2RGB)
        ...
        result = {
            ...
            "bev_raw": bev_raw,  # NEW: BEV camera
            "seg_front": seg_front,
            "seg_left": seg_left,
            "seg_right": seg_right,
        ...
        # EXTRACT AND VISUALIZE HD MAP
        rgb_with_hdmap = rgb.copy()
        hd_map_data = None
        
        if self.hd_map_enabled and self.hd_map_extractor is not None and hasattr(self, '_vehicle'):
            try:
                vehicle_transform = self._vehicle.get_transform()
                vehicle_location = vehicle_transform.location
                
                camera_location = carla.Location(x=1.3, y=0.0, z=2.3)
                camera_rotation = carla.Rotation(pitch=0.0, yaw=0.0, roll=0.0)
                camera_transform = carla.Transform(
                    vehicle_transform.location + camera_location,
                    vehicle_transform.rotation + camera_rotation
                )
                
                lane_boundaries = self.hd_map_extractor.get_lane_boundaries(
                    vehicle_location, forward_distance=50.0, lateral_distance=20.0
                )
                traffic_lights = self.hd_map_extractor.get_traffic_lights(vehicle_location, radius=50.0)
                traffic_signs = self.hd_map_extractor.get_traffic_signs(vehicle_location, radius=50.0)
                lane_info = self.hd_map_extractor.get_current_lane_info(vehicle_location, vehicle_transform.rotation)
                
                rgb_with_hdmap = rgb.copy()
                
                if lane_boundaries:
                    left_marking = lane_boundaries.get('left_marking_type', {})
                    right_marking = lane_boundaries.get('right_marking_type', {})
                    left_color = (0, 255, 255) if left_marking.get('type') == 'solid' else (255, 255, 255)
                    right_color = (0, 255, 255) if right_marking.get('type') == 'solid' else (255, 255, 255)
                    
                    rgb_with_hdmap = self.hd_map_visualizer.draw_lane_boundaries(
                        rgb_with_hdmap, lane_boundaries, camera_transform,
                        color_left=left_color, color_right=right_color,
                        color_center=(255, 255, 0), thickness=3
                    )
                
                for light in traffic_lights:
                    rgb_with_hdmap = self.hd_map_visualizer.draw_traffic_light(rgb_with_hdmap, light, camera_transform)
                
                for sign in traffic_signs:
                    rgb_with_hdmap = self.hd_map_visualizer.draw_traffic_sign(rgb_with_hdmap, sign, camera_transform)
                
                if lane_info:
                    rgb_with_hdmap = self.hd_map_visualizer.draw_lane_info(rgb_with_hdmap, lane_info, position=(10, 30))
                
                hd_map_data = {
                    'lane_boundaries': lane_boundaries,
                    'traffic_lights': traffic_lights,
                    'traffic_signs': traffic_signs,
                    'current_lane': lane_info,
                }
                
                result["has_hd_map"] = True
                result["hd_map_data"] = hd_map_data
                
                if self.step % 20 == 0:
                    print(f"Frame {self.step}: HD map extracted - {len(traffic_lights)} lights, {len(traffic_signs)} signs")
                    
            except Exception as e:
                if self.step % 100 == 0:
                    print(f"HD Map error: {e}")
        
        result["rgb_with_hdmap"] = rgb_with_hdmap
        ...
        rgb_with_tracks_frame = cv2.cvtColor(result["rgb_with_tracks"], cv2.COLOR_RGB2BGR)
        rgb_with_hdmap_frame = cv2.cvtColor(result["rgb_with_hdmap"], cv2.COLOR_RGB2BGR)
        
        # ============================================================
        # SAVE BEV CAMERA (NEW!)
        # ============================================================
        bev_frame = cv2.cvtColor(result["bev_raw"], cv2.COLOR_RGB2BGR)
        
        seg_front_frame = result["seg_front"]
        seg_front_colored_frame = cv2.cvtColor(result["seg_front_colored"], cv2.COLOR_RGB2BGR)
        seg_left_frame = result["seg_left"]
        seg_right_frame = result["seg_right"]
        
        inst_front_frame = result["inst_front"]
        inst_front_colored_frame = cv2.cvtColor(result["inst_front_colored"], cv2.COLOR_RGB2BGR)
        inst_left_frame = result["inst_left"]
        inst_right_frame = result["inst_right"]

        threads = [
            threading.Thread(target=cv2.imwrite, args=(os.path.join(FRAME_DIRS["rgb"], f"{self.step:06d}.jpg"), rgb_frame)),
            threading.Thread(target=cv2.imwrite, args=(os.path.join(FRAME_DIRS["rgb_left"], f"{self.step:06d}.jpg"), rgb_left_frame)),
            threading.Thread(target=cv2.imwrite, args=(os.path.join(FRAME_DIRS["rgb_right"], f"{self.step:06d}.jpg"), rgb_right_frame)),
            threading.Thread(target=cv2.imwrite, args=(os.path.join(FRAME_DIRS["rgb_with_hdmap"], f"{self.step:06d}.jpg"), rgb_with_hdmap_frame)),
            threading.Thread(target=cv2.imwrite, args=(os.path.join(FRAME_DIRS["bev"], f"{self.step:06d}.jpg"), bev_frame)),  # NEW
            threading.Thread(target=cv2.imwrite, args=(os.path.join(FRAME_DIRS["seg_front"], f"{self.step:06d}.png"), seg_front_frame)),
            threading.Thread(target=cv2.imwrite, args=(os.path.join(FRAME_DIRS["seg_left"], f"{self.step:06d}.png"), seg_left_frame)),
            threading.Thread(target=cv2.imwrite, args=(os.path.join(FRAME_DIRS["seg_right"], f"{self.step:06d}.png"), seg_right_frame)),
            threading.Thread(target=cv2.imwrite, args=(os.path.join(FRAME_DIRS["seg_colored_front"], f"{self.step:06d}.png"), seg_front_colored_frame)),
            threading.Thread(target=cv2.imwrite, args=(os.path.join(FRAME_DIRS["inst_front"], f"{self.step:06d}.png"), inst_front_frame)),
            threading.Thread(target=cv2.imwrite, args=(os.path.join(FRAME_DIRS["inst_left"], f"{self.step:06d}.png"), inst_left_frame)),
            threading.Thread(target=cv2.imwrite, args=(os.path.join(FRAME_DIRS["inst_right"], f"{self.step:06d}.png"), inst_right_frame)),
            threading.Thread(target=cv2.imwrite, args=(os.path.join(FRAME_DIRS["inst_colored_front"], f"{self.step:06d}.png"), inst_front_colored_frame)),
            threading.Thread(target=cv2.imwrite, args=(os.path.join(FRAME_DIRS["tracking_visualization"], f"{self.step:06d}.jpg"), rgb_with_tracks_frame)),
        ]

        for t in threads:
            t.start()
        
        analysis_path = os.path.join(FRAME_DIRS["inst_analysis"], f"{self.step:06d}.json")
        with open(analysis_path, 'w') as f:
            json.dump(result["instance_analysis"], f, indent=2)
        
        if tracking_results:
            tracking_path = os.path.join(FRAME_DIRS["tracking"], f"{self.step:06d}.json")
            tracking_export = {
                'frame': self.step,
                'timestamp': input_data['rgb'][0] if 'rgb' in input_data else 0.0,
                'tracked_objects': {
        ...
        if result.get("has_hd_map") and hd_map_data:
            hd_map_path = os.path.join(FRAME_DIRS["hd_map_data"], f"{self.step:06d}.json")
            serialized = serialize_hd_map_for_json(hd_map_data)
            with open(hd_map_path, 'w') as f:
    ...
    def run_step(self, input_data, timestamp):
        ...
        # ========================================
        # BROADCAST TO DIGITAL TWIN (WITH BEV!)
        # ========================================
        if self.twin_broadcaster.enabled and self.twin_broadcaster.connected:
            try:
                tracking_results = tick_data.get('tracking_results', {}) or {}
                instance_counts = tick_data.get('instance_analysis', {}).get('counts', {})
                hd_map_data = tick_data.get('hd_map_data')

                state_data = {
                    'timestamp': timestamp,
                    'step': self.step,
                    'vehicle_state': {
                        'position': tick_data['gps'].tolist() if hasattr(tick_data['gps'], 'tolist') else list(tick_data['gps']),
                    ...
                    'hd_map': serialize_hd_map_for_json(hd_map_data) if hd_map_data else None,
                    'has_hd_map': tick_data.get('has_hd_map', False),
                    'lidar': tick_data.get('raw_lidar'),
                    'seg_raw': tick_data.get('seg_front'),
                    'images': {
                        'rgb': compress_image(tick_data['rgb_raw']),
                        'rgb_left': compress_image(tick_data['rgb_left_raw']),
                        'rgb_right': compress_image(tick_data['rgb_right_raw']),
                        'rgb_with_tracks': compress_image(tick_data['rgb_with_tracks']),
                        'rgb_with_hdmap': compress_image(tick_data['rgb_with_hdmap']),
                        'bev': compress_image(tick_data['bev_raw']),  # NEW: BEV camera!
    ...
    def destroy(self):
        if hasattr(self, 'instance_tracker'):
            trajectories_path = os.path.join(DATASET_DIR, "complete_trajectories.json")
            ...
            all_tracks = {
                obj_id: {
                    'class_name': obj_info['class_name'],
                    'total_frames': obj_info['total_frames_visible'],
                    'first_frame': obj_info['first_seen_frame'],
                    'last_frame': obj_info['last_seen_frame']
                }
                for obj_id, obj_info in self.instance_tracker.objects.items()
            }
            
            stats = {
                'total_frames': self.instance_tracker.frame_count,
                'total_objects_tracked': len(all_tracks),
                'by_class': {},
                'long_tracks': []
            }
            
            for obj_id, track_info in all_tracks.items():
                class_name = track_info['class_name']
                ...
                stats['by_class'][class_name]['total_frames'] += track_info['total_frames']
                
                if track_info['total_frames'] > 30:
                    stats['long_tracks'].append({
                        'object_id': obj_id,
                        'class_name': class_name,
                        'duration_frames': track_info['total_frames']
                    })
            
            for class_name in stats['by_class']:
                count = stats['by_class'][class_name]['count']
                total = stats['by_class'][class_name]['total_frames']
                stats['by_class'][class_name]['avg_duration'] = total / count if count > 0 else 0
            
            stats_path = os.path.join(DATASET_DIR, "tracking_statistics.json")
            with open(stats_path, 'w') as f:
                json.dump(stats, f, indent=2)
            
            print(f"✓ Tracking statistics: {stats['total_objects_tracked']} objects")
            if stats['by_class']:
                print(f"  Classes tracked:")
                for cls, info in stats['by_class'].items():
                    print(f"    - {cls}: {info['count']} objects (avg {info['avg_duration']:.1f} frames)")
        ...
        dataset_info = {
            ...
            'has_hd_maps': self.hd_map_enabled,
            'has_bev_camera': True,  # NEW
            'note': 'Instance segmentation created from semantic segmentation using connected components. BEV camera added for top-down view.'
        }
        ...
        print(f"  Instance type: Pseudo (from semantic + connected components)")
        print(f"  HD Maps: {'Enabled' if self.hd_map_enabled else 'Disabled'}")
        print(f"  BEV Camera: Enabled (50m above vehicle)")
