...
# Added portion 1
DATASET_DIR = "/new_ssd/interfuser_complete_dataset"
FRAME_DIRS = {
...
class InterfuserAgent(autonomous_agent.AutonomousAgent):
    def setup(self, path_to_conf_file):
        ...
        # -----------------------
        self.attention_maps = {}
        os.makedirs("attention_images_1", exist_ok=True)
    ...
    def run_step(self, input_data, timestamp):
        ...
        #Added portion 4 (OPTIONAL - uncomment if you want BEV features)
        ...
        if self.attention_maps:
            for layer_name, attn_map in self.attention_maps.items():
                if len(attn_map.shape) >= 3:
                    attn_map_2d = np.mean(attn_map, axis=1)[0]
                else:
