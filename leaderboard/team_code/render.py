...
def add_rect(img, loc, ori, box, value, pixels_per_meter, max_distance, color):
    ...
    # color is expected as [b,g,r] in 0..1 or similar; value is usually 255
...
def render(det_data, pixels_per_meter=5, max_distance=18, t=0):
    """
    det_data: 20 x 20 x 7 interpretable map
    pixels_per_meter / max_distance control BEV scale (agent often calls with pixels_per_meter=20)
    """
    ...
    # ---- NEW: class-specific colors in BGR (scaled later by 'value') ----
    # Note: we pass colors in 0..1-ish and multiply by 255 in add_rect
    car_color = np.array([0.0, 1.0, 1.0])   # (B,G,R) ~ yellow/cyan-ish
    bike_color = np.array([1.0, 0.6, 0.0])  # orange-ish
    ped_color = np.array([0.0, 0.0, 1.0])   # red
    ...
    for poi in box_ids:
        ...
        # ---- NEW: choose color based on class ----
        if poi in box_info["car"]:
            color = car_color
        elif poi in box_info["bike"]:
            color = bike_color
        elif poi in box_info["pedestrian"]:
            color = ped_color
        else:
            # fallback
    ...
    # convert lists of positions → counts
