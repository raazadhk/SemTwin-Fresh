...
# Use environment variable or default to new SSD
...
FRAME_DIRS = {
    ...
    # Pseudo Instance Segmentation (from connected components)
...
def convert_segmentation_to_rgb(seg_image):
    ...
    color_map = {
        ...
        1: [70, 70, 70],      # Building - Dark Gray
        2: [100, 40, 40],     # Fence - Brown
        3: [55, 90, 80],      # Other - Teal
        4: [220, 20, 60],     # Pedestrian - Red
        5: [153, 153, 153],   # Pole - Light Gray
        6: [157, 234, 50],    # RoadLine - Lime
        7: [128, 64, 128],    # Road - Purple
        8: [244, 35, 232],    # Sidewalk - Pink
        9: [107, 142, 35],    # Vegetation - Green
        10: [0, 0, 142],      # Vehicles - Blue
        11: [102, 102, 156],  # Wall - Blue Gray
        12: [220, 220, 0],    # TrafficSign - Yellow
        13: [70, 130, 180],   # Sky - Light Blue
        14: [81, 0, 81],      # Ground - Dark Purple
        15: [150, 100, 100],  # Bridge - Light Brown
        16: [230, 150, 140],  # RailTrack - Salmon
        17: [180, 165, 180],  # GuardRail - Light Purple
        18: [250, 170, 30],   # TrafficLight - Orange
        19: [110, 190, 160],  # Static - Cyan
        20: [170, 120, 50],   # Dynamic - Brown Orange
        21: [45, 60, 150],    # Water - Dark Blue
        22: [145, 170, 100],  # Terrain - Olive
    }
...
def create_pseudo_instances_from_semantic(semantic_image):
    """
    Create pseudo instance segmentation using connected components
    from semantic segmentation. This simulates instance segmentation
    when the actual sensor is not available.
    """
    semantic_ids = semantic_image[:, :, 2]
    
    # Create pseudo instance image
    h, w = semantic_ids.shape
    instance_image = np.zeros((h, w, 3), dtype=np.uint8)
    
    # For each important class, find connected components
    # 4: Pedestrian, 10: Vehicle, 12: TrafficSign, 18: TrafficLight
    important_classes = [4, 10, 12, 18]
    
    instance_id = 1
    instance_map = {}  # maps instance_id to class_id
    ...
    for class_id in important_classes:
        ...
        for label in range(1, num_labels):  # Skip background (0)
            ...
            # Skip very small components (noise) - at least 100 pixels
            if np.sum(component_mask) < 100:
                continue
            
            # Encode instance ID in the image (similar to CARLA format)
            # R channel = instance_id % 256
            # G channel = instance_id // 256
...
def convert_instance_to_rgb(instance_image):
    """Convert instance segmentation to colored RGB image"""
    ...
    # Use seeded random for consistent colors per instance ID
...
def analyze_instances(instance_image, semantic_image):
    """Analyze instance segmentation to count objects by class"""
    instance_ids = instance_image[:, :, 2].astype(np.uint32) + \
                   instance_image[:, :, 1].astype(np.uint32) * 256
...
def draw_instance_boxes(rgb_image, instances_info):
    """Draw bounding boxes and labels on RGB image for detected instances"""
...
def draw_tracked_objects(rgb_image, tracking_results, show_trails=True):
    """Draw tracked objects with IDs, trajectories, and velocity"""
...
class InstanceTracker:
    ...
    def update(self, detections):
        ...
        if len(self.objects) == 0:
            ...
            # Return properly formatted tracking results for first frame
            tracking_results = {}
            for object_id, obj_info in self.objects.items():
                tracking_results[object_id] = {
                    ...
                    'velocity': {'vx': 0, 'vy': 0, 'speed': 0}  # No velocity on first frame
                }
        ...
        for (row, col) in zip(rows, cols):
            ...
            # For pseudo-instances, rely more on spatial proximity
            if distances[row, col] < self.max_distance:
    ...
    def _estimate_velocity(self, obj_info, window=5):
        """Estimate object velocity from recent history"""
...
def save_sensor_data_to_disk(step, tick_data, input_data, frame_dirs):
    ...
    measurements = {
        ...
        'instance_type': 'pseudo',  # Pseudo-instances from connected components
...
class InterfuserAgent(autonomous_agent.AutonomousAgent):
    ...
    def sensors(self):
        return [
            ...
            # Semantic Segmentation ONLY (instance segmentation not allowed in leaderboard)
    ...
    def tick(self, input_data):
        ...
        # CREATE PSEUDO INSTANCE SEGMENTATION FROM SEMANTIC (Connected Components)
        ...
        threads = [
            ...
            threading.Thread(
                target=cv2.imwrite,
                args=(os.path.join(FRAME_DIRS["seg_front"], f"{self.step:06d}.png"), seg_front_frame)
            ),
            threading.Thread(
                target=cv2.imwrite,
                args=(os.path.join(FRAME_DIRS["seg_left"], f"{self.step:06d}.png"), seg_left_frame)
            ),
            threading.Thread(
                target=cv2.imwrite,
                args=(os.path.join(FRAME_DIRS["seg_right"], f"{self.step:06d}.png"), seg_right_frame)
            ),
            threading.Thread(
                target=cv2.imwrite,
                args=(os.path.join(FRAME_DIRS["seg_colored_front"], f"{self.step:06d}.png"), seg_front_colored_frame)
            ),
            threading.Thread(
                target=cv2.imwrite,
                args=(os.path.join(FRAME_DIRS["inst_front"], f"{self.step:06d}.png"), inst_front_frame)
            ),
            threading.Thread(
                target=cv2.imwrite,
                args=(os.path.join(FRAME_DIRS["inst_left"], f"{self.step:06d}.png"), inst_left_frame)
            ),
            threading.Thread(
                target=cv2.imwrite,
                args=(os.path.join(FRAME_DIRS["inst_right"], f"{self.step:06d}.png"), inst_right_frame)
            ),
            threading.Thread(
                target=cv2.imwrite,
                args=(os.path.join(FRAME_DIRS["inst_colored_front"], f"{self.step:06d}.png"), inst_front_colored_frame)
            ),
            threading.Thread(
                target=cv2.imwrite,
                args=(os.path.join(FRAME_DIRS["tracking_visualization"], f"{self.step:06d}.jpg"), rgb_with_tracks_frame)
            ),
        ]
    ...
    def run_step(self, input_data, timestamp):
        ...
        input_data_model = {}
        input_data_model["rgb"] = rgb
        input_data_model["rgb_left"] = rgb_left
        input_data_model["rgb_right"] = rgb_right
        input_data_model["rgb_center"] = rgb_center
        input_data_model["measurements"] = mes
        input_data_model["target_point"] = (
            torch.from_numpy(tick_data["target_point"]).float().cuda().view(1, -1)
        )
        input_data_model["lidar"] = (
    ...
    def destroy(self):
        ...
        # Then continue with existing destroy code
        if self.ensemble:
            del self.nets
        else:
            del self.net
        ...
        dataset_info = {
            ...
            'note': 'Instance segmentation created from semantic segmentation using connected components'
        }
