...
def process_image(image_path):
    ...
    for box in results.boxes:
        ...
        if label in important_labels and confidence > 0.3:  # Confidence threshold
            ...
            # 🔹 Resize heatmap to match full image size (600x800)
            cam_map = cv2.resize(cam_map, (orig_size[0], orig_size[1]), interpolation=cv2.INTER_LINEAR)

            # 🔹 Normalize & Apply Gaussian Blur for smooth spreading
            cam_map = (cam_map - cam_map.min()) / (cam_map.max() - cam_map.min() + 1e-8)
            cam_map = np.power(cam_map, 0.4)  # Lower power spreads more
            cam_map = cv2.GaussianBlur(cam_map, (55, 55), 20)

            # 🔹 Spread heatmap over entire image
            full_heatmap = np.maximum(full_heatmap, cam_map)
    ...
    # Overlay heatmap onto full image
