...
def draw_bev_instances_enhanced(rgb_image, instances_info, image_size=(512, 512), ego_vehicle_marker=True):
    """
    Enhanced BEV visualization with MANDATORY ego vehicle at center in GOLD
    Other vehicles detected are shown in BLUE
    """
    image_with_annotations = rgb_image.copy()
    h, w = image_with_annotations.shape[:2]

    # Separate instances by class for organized drawing
    vehicles = []
    pedestrians = []
    traffic_lights = []
    traffic_signs = []
    ...
    for instance in instances_info['instances']:
        ...
        if class_name == 'vehicle':
            ...
            pedestrians.append(instance)
        elif class_name == 'traffic_light':
            traffic_lights.append(instance)
        elif class_name == 'traffic_sign':
            traffic_signs.append(instance)

    # ====================================================================
    # STEP 1: Draw OTHER vehicles FIRST (blue) - EXCLUDE center region
    # ====================================================================
    center_x, center_y = w // 2, h // 2
    ego_exclusion_radius = 30  # Exclude vehicles near center (ego position)
    
    for vehicle in vehicles:
        center = (vehicle['center_x'], vehicle['center_y'])
        bbox = vehicle['bbox']
        
        # Skip vehicles at/near the center (this is likely the ego vehicle detection)
        dist_from_center = np.sqrt((center[0] - center_x)**2 + (center[1] - center_y)**2)
        if dist_from_center < ego_exclusion_radius:
            continue  # Don't draw - this is where ego vehicle will be
        
        # Draw OTHER vehicles in BLUE
        cv2.rectangle(
            image_with_annotations,
            (bbox['x_min'], bbox['y_min']),
            (bbox['x_max'], bbox['y_max']),
            (255, 0, 0), 3  # BLUE for other vehicles
        )
        
        # Draw orientation dot
        cv2.circle(image_with_annotations, center, 4, (255, 0, 0), -1)

    # ====================================================================
    # STEP 2: Draw EGO VEHICLE at CENTER in GOLD (ALWAYS, MANDATORY)
    # ====================================================================
    if ego_vehicle_marker:
        # Ego vehicle dimensions (larger and prominent)
        ego_length = 50  # pixels (made larger)
        ego_width = 25
        
        # Draw filled rectangle for ego vehicle
        ego_rect_pts = np.array([
            [center_x - ego_width//2, center_y - ego_length//2],
            [center_x + ego_width//2, center_y - ego_length//2],
            [center_x + ego_width//2, center_y + ego_length//2],
            [center_x - ego_width//2, center_y + ego_length//2]
        ], np.int32)
        
        # GOLD/YELLOW fill for ego vehicle - VERY DISTINCTIVE
        cv2.fillPoly(image_with_annotations, [ego_rect_pts], (0, 215, 255))  # Gold fill
        cv2.polylines(image_with_annotations, [ego_rect_pts], True, (0, 140, 255), 4)  # Dark gold outline (thicker)
        
        # Draw direction arrow (GREEN - very visible)
        arrow_start = (center_x, center_y)
        arrow_end = (center_x, center_y - ego_length//2 - 15)
        cv2.arrowedLine(image_with_annotations, arrow_start, arrow_end,
                       (0, 255, 0), 4, tipLength=0.5)  # Thicker green arrow
        
        # Label for ego vehicle (LARGE and BOLD)
        cv2.putText(
            image_with_annotations, 'EGO',
            (center_x - 25, center_y + ego_length//2 + 25),
            cv2.FONT_HERSHEY_SIMPLEX, 0.8,  # Larger font
            (0, 215, 255), 3  # Thicker text
        )
        
        # Draw coordinate axes
        # X-axis (right) in RED
        cv2.line(image_with_annotations, 
                (center_x, center_y), 
                (center_x + 30, center_y),
                (0, 0, 255), 2)  # Red
        cv2.putText(image_with_annotations, 'X', (center_x + 33, center_y + 5),
                   cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 0, 255), 2)
        
        # Y-axis (forward) in GREEN
        cv2.line(image_with_annotations, 
                (center_x, center_y), 
                (center_x, center_y - 30),
                (0, 255, 0), 2)  # Green
        cv2.putText(image_with_annotations, 'Y', (center_x + 5, center_y - 33),
                   cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 255, 0), 2)

    # Draw pedestrians (RED circles with person icon)
    for pedestrian in pedestrians:
        center = (pedestrian['center_x'], pedestrian['center_y'])
        bbox = pedestrian['bbox']
        
        # Draw circle for pedestrian
        radius = max(7, min((bbox['x_max'] - bbox['x_min']), 
                           (bbox['y_max'] - bbox['y_min'])) // 2)
        cv2.circle(image_with_annotations, center, radius, (0, 0, 255), 2)  # RED
        
        # Draw head
        cv2.circle(image_with_annotations, 
                  (center[0], center[1] - radius//2), 
                  radius//3, (0, 0, 255), -1)
        
        # Label
        cv2.putText(
            image_with_annotations, 'P',
            (center[0] - 5, center[1] + radius + 15),
            cv2.FONT_HERSHEY_SIMPLEX, 0.5,
            (0, 0, 255), 2
        )

    # Draw traffic lights (CYAN with light icon)
    for tlight in traffic_lights:
        center = (tlight['center_x'], tlight['center_y'])
        bbox = tlight['bbox']
        
        # Draw traffic light pole and housing
        pole_bottom = (center[0], bbox['y_max'])
        pole_top = (center[0], bbox['y_min'])
        cv2.line(image_with_annotations, pole_bottom, pole_top, (255, 255, 0), 2)  # Cyan
        
        # Draw light housing (rectangle)
        light_w = min(15, (bbox['x_max'] - bbox['x_min']))
        light_h = min(25, (bbox['y_max'] - bbox['y_min']))
        cv2.rectangle(
            image_with_annotations,
            (center[0] - light_w//2, bbox['y_min']),
            (center[0] + light_w//2, bbox['y_min'] + light_h),
            (255, 255, 0), 2
        )
        ...
        circle_r = 3
        cv2.circle(image_with_annotations, 
                  (center[0], bbox['y_min'] + 5), circle_r, (0, 0, 200), -1)  # Red
        cv2.circle(image_with_annotations, 
                  (center[0], bbox['y_min'] + 12), circle_r, (0, 200, 200), -1)  # Yellow
        cv2.circle(image_with_annotations, 
                  (center[0], bbox['y_min'] + 19), circle_r, (0, 200, 0), -1)  # Green
        
        # Label
        cv2.putText(
            image_with_annotations, 'TL',
            (center[0] - 10, bbox['y_max'] + 15),
            cv2.FONT_HERSHEY_SIMPLEX, 0.4,
            (255, 255, 0), 1
        )

    # Draw traffic signs (GREEN with sign icon)
    for sign in traffic_signs:
        center = (sign['center_x'], sign['center_y'])
        bbox = sign['bbox']
        
        # Draw sign pole
        pole_bottom = (center[0], bbox['y_max'])
        pole_top = (center[0], center[1])
        cv2.line(image_with_annotations, pole_bottom, pole_top, (0, 255, 0), 2)
        
        # Draw sign board (octagon for stop sign shape)
        sign_size = min(12, (bbox['x_max'] - bbox['x_min'])//2)
        pts = []
        for i in range(8):
            angle = i * np.pi / 4
            x = int(center[0] + sign_size * np.cos(angle))
            y = int(bbox['y_min'] + sign_size * np.sin(angle) + sign_size)
            pts.append([x, y])
        pts = np.array(pts, np.int32)
        cv2.polylines(image_with_annotations, [pts], True, (0, 255, 0), 2)
        
        # Label
        cv2.putText(
            image_with_annotations, 'SIGN',
            (center[0] - 15, bbox['y_max'] + 15),
            cv2.FONT_HERSHEY_SIMPLEX, 0.35,
            (0, 255, 0), 1
        )

    # Add statistics overlay in corner
    stats_y = 20
    cv2.rectangle(image_with_annotations, (5, 5), (180, 110), (0, 0, 0), -1)
    cv2.rectangle(image_with_annotations, (5, 5), (180, 110), (255, 255, 255), 2)
    
    cv2.putText(image_with_annotations, 'BEV Detection:', (10, stats_y),
                cv2.FONT_HERSHEY_SIMPLEX, 0.45, (255, 255, 255), 1)
    stats_y += 20
    
    # Count other vehicles (excluding ego)
    other_vehicles = [v for v in vehicles 
                     if np.sqrt((v['center_x'] - center_x)**2 + 
                               (v['center_y'] - center_y)**2) >= ego_exclusion_radius]
    
    cv2.putText(image_with_annotations, f'Other Vehicles: {len(other_vehicles)}', (10, stats_y),
                cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 0, 0), 1)  # Blue for other vehicles
    stats_y += 18
    cv2.putText(image_with_annotations, f'Pedestrians: {len(pedestrians)}', (10, stats_y),
                cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 0, 255), 1)  # Red
    stats_y += 18
    cv2.putText(image_with_annotations, f'Traffic Lights: {len(traffic_lights)}', (10, stats_y),
                cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 255, 0), 1)  # Cyan
    stats_y += 18
    cv2.putText(image_with_annotations, f'Signs: {len(traffic_signs)}', (10, stats_y),
                cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 255, 0), 1)  # Green
...
class InterfuserAgent(autonomous_agent.AutonomousAgent):
    ...
    def tick(self, input_data):
        ...
        # Use ENHANCED BEV visualization with traffic infrastructure
        bev_sem_with_boxes = draw_bev_instances_enhanced(
            bev_sem_colored, 
            bev_instance_analysis,
            image_size=(512, 512)
        )
    ...
    def run_step(self, input_data, timestamp):
        ...
        # BROADCAST TO DIGITAL TWIN (including BEV HD map with traffic infrastructure)
        if self.twin_broadcaster.enabled and self.twin_broadcaster.connected:
            try:
                tracking_results = tick_data.get('tracking_results', {}) or {}
                instance_counts = tick_data.get('instance_analysis', {}).get('counts', {})
                
                # BEV-specific counts for traffic infrastructure
                bev_instance_counts = tick_data.get('bev_instance_analysis', {}).get('counts', {})
                ...
                state_data = {
                    ...
                    'bev_detections': {
                        'vehicles': bev_instance_counts.get('vehicles', 0),
                        'pedestrians': bev_instance_counts.get('pedestrians', 0),
                        'traffic_lights': bev_instance_counts.get('traffic_lights', 0),
                        'traffic_signs': bev_instance_counts.get('traffic_signs', 0),
                        'total_objects': bev_instance_counts.get('total_objects', 0),
                    },
                    'images': {
