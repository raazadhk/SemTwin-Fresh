...
# Save path for evaluation logs (unchanged)
SAVE_PATH = os.environ.get("SAVE_PATH", 'eval')
IMAGENET_DEFAULT_MEAN = (0.485, 0.456, 0.406)
IMAGENET_DEFAULT_STD = (0.229, 0.224, 0.225)

# -------------------------------
# Dataset Directories for Frames
# -------------------------------
# This directory will store frames from the three RGB cameras.
...
class InterfuserAgent(autonomous_agent.AutonomousAgent):
    ...
    def sensors(self):
        return [
            {"type": "sensor.camera.rgb", "x": 1.3, "y": 0.0, "z": 2.3, "roll": 0.0, "pitch": 0.0, "yaw": 0.0, "width": 800, "height": 600, "fov": 100, "id": "rgb"},
    ...
    def tick(self, input_data):
        ...
        if math.isnan(compass):
            compass = 0.0
        result = {"rgb": rgb, "rgb_left": rgb_left, "rgb_right": rgb_right, "gps": gps, "speed": speed, "compass": compass}
        pos = self._get_position(result)
        lidar_data = input_data['lidar'][1]
        result['raw_lidar'] = lidar_data
        lidar_unprocessed = lidar_data[:, :3]
        lidar_unprocessed[:, 1] *= -1
        full_lidar = transform_2d_points(lidar_unprocessed, np.pi / 2 - compass, -pos[0], -pos[1],
        ...
        # --- Save frames for dataset ---
        # Save each camera frame as an image (converted to BGR for OpenCV saving)
        ...
        # Save images using the current step as frame number
    ...
    def run_step(self, input_data, timestamp):
        ...
        if self.ensemble:
            ...
            with torch.no_grad():
                (traffic_meta, pred_waypoints, is_junction, traffic_light_state, stop_sign, bev_feature) = self.net(input_data)
        ...
        steer, throttle, brake, meta_infos = self.controller.run_step(velocity, pred_waypoints, is_junction, traffic_light_state, stop_sign, self.traffic_meta_moving_avg)
        if brake < 0.05:
            brake = 0.0
        if brake > 0.1:
            throttle = 0.0
        ...
        wp = wp1 + wp2
        surround_map = np.clip((surround_map.astype(np.float32) + self_car_map.astype(np.float32) + wp.astype(np.float32)), 0, 255).astype(np.uint8)
